...
import { Session, SessionDocument, SessionPSQL } from '../../sessions.models';
import { InjectDataSource } from '@nestjs/typeorm';
import { DataSource } from 'typeorm';
import { ISessionsRepository } from '../../../auth/Service/auth.service';

@Injectable()
export class SessionsRepositoryRawPSQL implements ISessionsRepository {
  ...
  async getSessionByDeviceId(deviceId: string): Promise<SessionPSQL | null> {
    const result = await this.dataSource.query<SessionPSQL[]>(
      `
  ...
  async save(session: SessionPSQL) {
    return (
      await this.dataSource.query<SessionPSQL[]>(
        `INSERT INTO "Sessions"
            ("IP", title, "lastActiveDate", "expDate", "deviceId", "userId")
            VALUES ($1,$2,$3,$4,$5,$6)
            RETURNING *`,
        [
          session.ip,
